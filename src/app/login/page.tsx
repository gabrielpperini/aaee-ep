  ...
  const subtitle =
    mode === "password"
      ...
      : mode === "otp-request"
        ...
        : mode === "otp-verify"
          ? `Digite o código de 6 dígitos enviado para ${sharedEmail}.`
          : mode === "forgot-request"
            ? "Te enviamos um link para criar uma senha nova."
            : `Abra o link enviado para ${sharedEmail} para criar uma nova senha.`;
        ...
        <div className="w-full max-w-md rise-in">
          ...
          {mode === "password" && (
            <PasswordForm
              ...
              onSwitchToForgot={() => setMode("forgot-request")}
              onSuccess={() => {
          ...
          {mode === "forgot-sent" && (
            <div className="space-y-4 text-sm">
              <p className="text-muted-foreground">
                ...
                ao clicar, você cai numa tela para criar a senha nova.
              </p>
              <Button
                type="button"
                variant="outline"
  ...
  const submit = form.handleSubmit((values) => {
    startTransition(async () => {
      ...
      toast.success("Código enviado", {
        description: `Verifique a caixa de entrada de ${values.email}.`,
      });
  ...
  const submit = form.handleSubmit((values) => {
    startTransition(async () => {
      ...
      if (error) {
        ...
        form.setError("token", { message: "Código inválido" });
        return;
      }
      onSuccess();
    });
...
function ForgotForm({
  ...
  const submit = form.handleSubmit((values) => {
    startTransition(async () => {
      ...
      if (error) {
        toast.error("Não consegui enviar o email", {
          description: error.message,
        });
        return;
      }
    ...
    <aside className="relative hidden lg:flex w-[44%] max-w-[640px] flex-col justify-between overflow-hidden bg-sidebar text-sidebar-foreground p-10">
      ...
      <div className="relative z-10 max-w-sm">
        ...
        <p className="mt-5 text-sm text-sidebar-foreground/75 text-pretty">
          A plataforma da torcida da Engenharia UFRGS no EP: agenda, atletas,
          locais e a operação do grito numa só tela.
        </p>
      </div>
